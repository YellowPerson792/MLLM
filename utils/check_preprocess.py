...
# 修改 dataset 参数选项，支持 flickr8k
parser.add_argument('--dataset', type=str, default='cifar10', choices=['mnist', 'cifar10', 'imagenet100', 'flickr8k'], help='Dataset to process')
...
if args.dataset in ['mnist', 'cifar10', 'imagenet100', 'flickr8k']:
    ...
    if args.dataset == 'flickr8k':
        ...
        for label in range(10):
            ...
            sample_txt_lines.append(
                f"Sample {chosen_idx} label={label}\nJPEG hex: {jpeg_hex}\nToken ids: {input_ids}\n\n"
            )
        if len(category_saved) < 10:
            raise ValueError(
                f"前5000样本仅包含类别: {sorted(category_saved)}，不足10类，请扩大样本范围！"
            )
        with open(save_txt_path, 'w', encoding='utf-8') as ftxt:
            ftxt.writelines(sample_txt_lines)
...
for i, ex in enumerate(examples_2000):
    ...
    if not args.filter or actual_len <= FILTER_THRESHOLD:
        ...
        # 仅分类数据集统计类别分布
        if args.dataset in ['mnist', 'cifar10', 'imagenet100']:
...
if lengths_2000:
    ...
    if args.dataset == 'imagenet100':
        ...
        step_size = 200  # mnist、cifar10、flickr8k
