...
from transformers import EncoderDecoderModel, ViTModel, GPT2LMHeadModel, ViTFeatureExtractor, AutoTokenizer, Seq2SeqTrainingArguments, Seq2SeqTrainer, default_data_collator, GenerationConfig, GPT2Config
...
class JpegBytesDataset(Dataset):
    def __init__(self, df, root_dir, tokenizer, max_length=2000, image_size=224, bit_flip_prob=0.0):
    ...
    def __getitem__(self, idx):
        ...
        input_ids = [self.tokenizer.bos_token_id] + self.tokenizer(jpeg_str, add_special_tokens=False)["input_ids"]
...
train_dataset = JpegBytesDataset(train_df, img_root, tokenizer, image_size=config.MAX_LEN)
val_dataset = JpegBytesDataset(val_df, img_root, tokenizer, image_size=config.MAX_LEN)
...
wrapped_vit = ViTEncoderWrapper.from_pretrained(config.ENCODER)
model = EncoderDecoderModel(encoder=wrapped_vit, decoder=gpt2)
# 只设置结构/训练相关参数
...
def compute_metrics(pred):
    ...
    label_str = tokenizer.batch_decode(labels_ids.tolist(), skip_special_tokens=True)
...
parser.add_argument('--output_dir', type=str, default=os.path.join(script_dir, "VIT_GPT2_EDM"))
...
def generate_caption(image_path):
    ...
    pixel_values = preprocess(images=img, return_tensors="pt").pixel_values.to(model.device)
    encoder_outputs = model.get_encoder()(pixel_values=pixel_values)
