...
def main():
    ...
    parser.add_argument("--model_name_or_path", type=str, default="/root/autodl-tmp/MLLM/models/jpeg-lm", help="")
    ...
    if args.query_vllm_server:  # Remember to export VLLM_WORKER_MULTIPROC_METHOD=spawn when using vLLM
        from vllm import LLM, SamplingParams # version 0.3.3
        if args.query_vllm_server == "local":
            model = LLM(model=args.model_name_or_path, max_model_len=MAX_MODEL_SEQ_LEN, max_seq_len_to_capture=MAX_MODEL_SEQ_LEN, dtype='float16')
    else:
    ...
    for _image_path in image_path:
        ...
        reader.append({'image': preprocess(image), 'path': _image_path})
    if args.output_dir:
    ...
    for _i, batch in enumerate(reader):
        ...
        # 获取输入图片的文件名（不含扩展名）
        input_filename = os.path.splitext(os.path.basename(batch['path']))[0]
        ...
        save_byte_image(image_str, os.path.join(args.output_dir, f"original_{input_filename}_{config_str}"))

        # partial image (prompt)
        save_byte_image(clean_image_string(partial_input_string), os.path.join(args.output_dir, f"partial_{input_filename}_{config_str}"))

        # generation
        for _j, output_string in enumerate(output_string_list):
            save_byte_image(clean_image_string(output_string), os.path.join(args.output_dir, f"generation_{input_filename}-{_j}_{config_str}"))
